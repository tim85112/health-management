...
public class EnrollmentController {
    ...
    @Operation(summary = "查詢所有常規課程並包含當前使用者報名狀態和人數")
    @GetMapping("/courses/all/with-status")
    public ResponseEntity<?> getAllCoursesWithUserEnrollmentStatus(@AuthenticationPrincipal UserDetails userDetails) {
        Integer userId = null;
        if (userDetails != null) {
            String email = userDetails.getUsername();
            ...
            if (userOptional.isPresent()) {
                 ...
                 logger.info("已認證使用者 (Email: {}) 查詢所有課程和報名狀態。", email);
            } else {
                 logger.warn("已認證使用者 (Email: {}) 找不到對應 User 實體，無法查詢個人報名狀態。", email);
            }
        } else {
            logger.info("匿名使用者查詢所有課程和報名狀態 (不含個人狀態)。");
        }
        try {
            List<CourseInfoDTO> coursesWithStatus = enrollmentService.getAllCoursesWithUserStatus(userId);
            logger.info("成功獲取 {} 條課程及報名狀態信息。", coursesWithStatus.size());
            return ResponseEntity.ok(coursesWithStatus);
        } catch (Exception e) {
             logger.error("查詢所有課程和報名狀態時發生內部錯誤", e);
            return ResponseEntity.status(HttpStatus.INTERNAL_SERVER_ERROR).body(new ErrorResponse("獲取課程信息失敗。"));
        }
    }
