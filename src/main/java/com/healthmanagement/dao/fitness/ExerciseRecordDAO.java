...
public interface ExerciseRecordDAO
		extends JpaRepository<ExerciseRecord, Integer>, JpaSpecificationExecutor<ExerciseRecord> {
	...
	@Query("SELECT er FROM ExerciseRecord er " + "WHERE er.userId = :userId "
			+ "AND er.exerciseDate BETWEEN :startDate AND :endDate")
	List<ExerciseRecord> findByUserIdAndExerciseDateBetween(@Param("userId") Integer userId,
